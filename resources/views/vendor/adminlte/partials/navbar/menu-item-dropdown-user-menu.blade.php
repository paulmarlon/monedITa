...
@php
    ...
    $umValueOf = static function ($method) use ($umUser) {
        return is_object($umUser) && method_exists($umUser, $method) ? $umUser->{$method}() : null;
    };

    $umName = $umUser->alias ?? ($umUser->name ?? '');
...
--}}
@php
    $umBsColors = ['primary', 'secondary', 'success', 'danger', 'warning', 'info', 'light', 'dark'];
...
<li class="nav-item dropdown user-menu">
    ...
    <a href="#" class="nav-link dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
        @if ($umImage)
            <img src="{{ $umImage }}" class="user-image rounded-circle shadow" alt="{{ $umName }}">
        @endif
    ...
    <ul class="dropdown-menu dropdown-menu-lg dropdown-menu-end">
        ...
        @if (!View::hasSection('usermenu_header') && config('adminlte.usermenu_header'))
            <li class="user-header {{ $umHeaderClass }}"
                ...
                @if ($umImage)
                    <img src="{{ $umImage }}" class="rounded-circle shadow" alt="{{ $umName }}">
                @endif
        ...
        <li class="user-footer">
            ...
            @endif
            <a class="btn btn-outline-danger float-end @if (!$profile_url) w-100 @endif" href="#"
